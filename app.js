...
sortedByBirthYear=inventors.sort((old,recent)=> old.year-recent.year);
...
firstLast=people.map(person => { let [last,first]=person.split(",");
return `${first} ${last}`
});
...
isAdultPresent = devs.some(adult=> 2025-adult.yeare>=18);
// Check your work:
...
isEveryone19OrOlder = devs.every(older=> 2025-older.year>=19);
...
commentById = comments.find(where =>where.id === 823423);
...
idx = comments.findIndex(look => look.id === 123523);
